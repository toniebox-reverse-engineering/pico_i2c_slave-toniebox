...
void loop() {
    if (get_bootsel_button()) {
        ...
        if (get_bootsel_button()) { //Long
            ...
            //TODO
            puts("Press short");
            ctx.mem[PULSE_SRC] = 0b00000000; //00 -- ??? -real
            ctx.mem[PULSE_SRC] = 0b10100010; //a2 -- crash -real
            ctx.mem[PULSE_SRC] = 0b11000000; //c0 -- ok -real
            ctx.mem[PULSE_SRC] = 0b11000100; //c4 -- ok -real
            ctx.mem[PULSE_SRC] = 0b11010000; //d0 -- read 0x81
            ctx.mem[PULSE_SRC] = 0b11100000; //e0 -- read 0x81
            ctx.mem[PULSE_SRC] = 0b11110000; //f0 -- crash
            ctx.mem[PULSE_SRC] = 0b11101000; //e8 -- crash
            ctx.mem[PULSE_SRC] = 0b11100100; //e4 -- crash
            ctx.mem[PULSE_SRC] = 0b11100010; //e2 -- crash
            ctx.mem[PULSE_SRC] = 0b11100001; //e1 -- crash
            if (lastTap) {
                //WORKS!!!!
                ctx.mem[STATUS_MMA8452Q] = 0xe0; //New data
                //ctx.mem[PULSE_SRC] = 0b11100010; //Left
                ctx.mem[PULSE_SRC] = 0xc0;
                uint8_t fifo[] = { 0xee, 0x0, 0x0, 0xef, 0xff, 0xff, 0xef, 0x0, 0xff, 0xef, 0x0, 0xfe, 0xef, 0x0, 0xff, 0xef, 0x0, 0xff, 0xef, 0x1, 0xff, 0xef, 0x1, 0xff, 0xf0, 0x1, 0xff, 0xf0, 0x1, 0x0, 0xf0, 0x1, 0xff, 0xef, 0x2, 0xff, 0xeb, 0x3, 0x0, 0xf2, 0x3, 0x0, 0xf4, 0x5e, 0x59, 0xfa, 0x7f, 0x12, 0x1, 0x5a, 0x10, 0xdd, 0x33, 0xf6, 0xd8, 0xfd, 0x12, 0xf0, 0xee, 0xf6, 0xeb, 0xe9, 0xfc, 0xec, 0xff, 0xeb, 0xf7, 0xef, 0xfc, 0xff, 0xe7, 0x0, 0xf8, 0xf8, 0xfc, 0xe9, 0xf0, 0x8, 0xe9, 0xeb, 0x7, 0xec, 0xee, 0x0, 0xeb, 0xed, 0xfe, 0xea, 0xf0, 0xfd, 0xee, 0xf2, 0xfd, 0xf0, 0xf4, 0xfd };
                memcpy(&ctx.mem[FIFO_BUF], fifo, sizeof(fifo));
            } else {
                ctx.mem[STATUS_MMA8452Q] = 0x05; //New data
                //ctx.mem[PULSE_SRC] = 0b11100100; //Right
                ctx.mem[PULSE_SRC] = 0x0;
                uint8_t fifo[] = { 0xf0, 0x0, 0x0, 0xf0, 0x1, 0xff, 0xef, 0x1, 0xff, 0xf0, 0x0, 0xff, 0xef, 0x0, 0xff, 0xef, 0x0, 0xff, 0xef, 0x0, 0xff, 0xf0, 0x0, 0xff, 0xf0, 0x0, 0xff, 0xf0, 0x0, 0xff, 0xef, 0x0, 0xff, 0xf1, 0xda, 0xda, 0xf4, 0xdb, 0xbe, 0xe4, 0xd6, 0x9b, 0x6, 0xd4, 0x92, 0xf9, 0x9, 0x5, 0xec, 0xe8, 0x5, 0xe2, 0x5, 0xfd, 0xe3, 0x46, 0x33, 0xe8, 0xf, 0x2f, 0xf2, 0xf5, 0x1d, 0xf3, 0xfd, 0xf, 0xf9, 0xfb, 0x19, 0xf7, 0xf3, 0x14, 0xf7, 0xf7, 0xd, 0xf5, 0xf6, 0x11, 0xef, 0xf8, 0xd, 0xe9, 0xfc, 0xb, 0xe7, 0x1, 0x8, 0xea, 0x2, 0x5, 0xeb, 0x5, 0x3, 0xec, 0x7, 0xff };
                memcpy(&ctx.mem[FIFO_BUF], fifo, sizeof(fifo));
            }
            lastTap = !lastTap;
        }
