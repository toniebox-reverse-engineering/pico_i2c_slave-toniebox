...
#define FORCE_SLEEP_ENABLED true 
bool sleep_pin_in_use;
...
static void setup_slave() {
    ...
    sleep_pin_in_use = FORCE_SLEEP_ENABLED;
}
...
#define SLEEP_TIMEOUT 1000 * 2
#define MESSAGE_TIMEOUT 1000 * 2
...
void loop() {
    ...
    if (lastMessageTime + MESSAGE_TIMEOUT < millis()) {
    ...
    bool sleep_gpio = gpio_get(SLEEP_PIN);
    if (sleep_gpio) {
        sleep_pin_in_use = true; 
        lastSleepTime = millis();
    } else if (sleep_pin_in_use && !sleep_gpio) {
        if (lastSleepTime + SLEEP_TIMEOUT < millis()) {
            printf("Let's sleep!\r\n");
            for (uint8_t i = 0; i < 10; i++) {
                led_switch();
                sleep_ms(20);
